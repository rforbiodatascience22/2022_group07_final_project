...
one_hot_enc_data_join <- PCA_data %>% 
  mutate(one_hot_enc_data$Population ,one_hot_enc_data$Sex)
```

#renaming columns
```{r}
one_hot_enc_data_join <- one_hot_enc_data_join %>% 
  rename( "Population" = "one_hot_enc_data$Population","Sex" = "one_hot_enc_data$Sex")
...
dummy <- dummyVars(" ~ .", data=one_hot_enc_data_join)

#perform one-hot encoding on data frame
final_data <- data.frame(predict(dummy, newdata=one_hot_enc_data_join))
...
a<-fviz_contrib(pca_fit, "var", axes=1, xtickslab.rt=90) # default angle=45°
...
pl3 <- pca_fit %>%
  augment(my_data_clean_aug) %>% # add original dataset back in
  ggplot(aes(.fittedPC1, .fittedPC2, color = distance_class)) + 
  geom_point(size = 1.5)
show(pl1 / pl3 + pl2)
```
...
```{r}
kmean <- pca_fit$x %>%
  kmeans(centers = 2, iter.max = 1000, nstart = 10) %>%
  augment(final_data)
